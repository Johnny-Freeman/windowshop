...
def wait_MarketOpen(d_min = 0):
	'''
		Block thread until market is open
	'''
	# Wait for market to open
	if d_min ==0:
		print('Waiting for Market Open')
	else:
		print'Waiting for Market Open, with d_min: ',d_min

	ddmin = CONFIG_MARKETHOURS['MarketOpen'].minute + d_min
	waitTime(CONFIG_MARKETHOURS['MarketOpen'].replace(minute=ddmin))

def wait_MarketClose(d_min = 0):
	'''
		Block thread until market is Closed
	'''
	# Wait for market to close
	if d_min ==0:
		print('Waiting for Market Close')
	else:
		print'Waiting for Market Close, with d_min: ',d_min
	
	ddmin = CONFIG_MARKETHOURS['MarketClose'].minute + d_min
	waitTime(CONFIG_MARKETHOURS['MarketClose'].replace(minute=ddmin))
